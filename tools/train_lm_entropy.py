...
def main() -> None:
    ...
    train_ds = RowTokenDataset(train_rows, label_bos_idx=no_above_idx)
    val_ds = RowTokenDataset(val_rows, label_bos_idx=no_above_idx)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            train_pixels += sum(int(m.H * m.W) for m in batch["meta"])
            n_batches += 1
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_pixels += sum(int(m.H * m.W) for m in batch["meta"])
                val_batches += 1
