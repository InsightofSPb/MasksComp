...
from maskscomp.entropy_coding.lm_codec import checkpoint_sha, decode_mask, load_checkpoint_and_model, read_bin
...
def main() -> None:
    ...
    ckpt, model = load_checkpoint_and_model(args.checkpoint, use_2d, torch.device(args.device))
    ...
    if args.verify_against is not None:
        ...
        if gt.shape != mask.shape or gt.dtype != mask.dtype or not np.array_equal(gt, mask):
