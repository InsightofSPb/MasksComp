...
def test_arith_roundtrip_online_random_mask(tmp_path: Path) -> None:
    rng = np.random.default_rng(123)
    mask = rng.integers(0, 5, size=(64, 64), dtype=np.uint8)
    inp = tmp_path / "rand.png"
    assert cv2.imwrite(str(inp), mask)

    ckpt = _make_ckpt(tmp_path / "best.pt")
    out_bin = tmp_path / "rand.bin"
    online = OnlineConfig(
        mode="online", lr=1e-3, steps_per_row=1, clip=1.0, online_after="row"
    )
