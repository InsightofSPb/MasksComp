...
def probs_to_cdf(probs: np.ndarray, allowed_mask: Optional[np.ndarray] = None, total_freq: int = 1 << 16) -> np.ndarray:
    probs = np.asarray(probs, dtype=np.float64)
    if probs.ndim != 1:
        raise ValueError("probs must be 1D")
    v = probs.shape[0]
    allowed = np.ones((v,), dtype=bool) if allowed_mask is None else np.asarray(allowed_mask, dtype=bool)
    if allowed.shape != (v,):
    ...
    if delta > 0:
        ...
        if int(freq.sum()) != int(total_freq):
            diff = int(total_freq - int(freq.sum()))
            for j in np.where(allowed)[0][: abs(diff)]:
                freq[j] += 1 if diff > 0 else -1
...
def _build_model(ckpt: dict, use_2d_context: bool, device: torch.device) -> LMEntropyModel:
    cfg = ckpt["config"]
    wmax = int(cfg["wmax"])
...
def _next_probs(model: LMEntropyModel, prefix_tokens: List[int], prefix_types: List[int], above_label: List[int], above_same: List[int], use_2d_context: bool, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
    ...
    with torch.no_grad():
        llog, lenlog = model(inp, ttypes, pad, above_label=al if use_2d_context else None, above_same=aeq if use_2d_context else None)
    return (
        F.softmax(llog[0, -1], dim=-1).detach().cpu().numpy(),
        F.softmax(lenlog[0, -1], dim=-1).detach().cpu().numpy(),
    )


def _online_update_row(model: LMEntropyModel, optimizer: torch.optim.Optimizer, tokens: np.ndarray, token_types: np.ndarray, rem_width: np.ndarray, above_label: np.ndarray, above_same: np.ndarray, no_above_idx: int, clip: float, use_2d_context: bool, device: torch.device) -> None:
    ...
    llog, lenlog = model(inp, ttypes, pad, above_label=al_t if use_2d_context else None, above_same=aeq_t if use_2d_context else None)
    target = inp[:, 1:]
    ttype = ttypes[:, 1:]
    remw = rem_t[:, 1:]
...
def encode_mask(mask: np.ndarray, model: LMEntropyModel, labels: Sequence[int], label_to_idx: Dict[int, int], use_2d_context: bool, online: OnlineConfig, device: torch.device) -> Tuple[bytes, float, float]:
    ...
    for row in rows:
        ...
        mapped[row.token_types == 0] = np.vectorize(label_to_idx.__getitem__)(mapped[row.token_types == 0])
        ...
        for j in range(len(row.labels)):
            ...
            lab = int(row.labels[j])
            ab = int(mask[row.y - 1, x])
            above_label[2 * j] = label_to_idx[ab]
            above_same[2 * j] = 1 if ab == lab else 0
        ...
        for t, sym in enumerate(mapped.tolist()):
            ...
            label_probs, len_probs = _next_probs(model, prefix_tokens, prefix_types, prefix_al, prefix_as, use_2d_context, device)
            if next_type == 0:
        ...
        if online.mode == "online":
            deferred_rows.append((mapped, row.token_types, row.rem_width, above_label, above_same))
            if online.online_after == "row":
                for _ in range(online.steps_per_row):
                    _online_update_row(model, optimizer, mapped, row.token_types, row.rem_width, above_label, above_same, no_above_idx, online.clip, use_2d_context, device)

    if online.mode == "online" and online.online_after == "image":
        for mapped, token_types, rem_width, above_label, above_same in deferred_rows:
            for _ in range(online.steps_per_row):
                _online_update_row(model, optimizer, mapped, token_types, rem_width, above_label, above_same, no_above_idx, online.clip, use_2d_context, device)

    enc.finish()
    return sink.getvalue(), ideal_bits, ideal_len_bits


def decode_mask(payload: bytes, header: dict, model: LMEntropyModel, use_2d_context: bool, device: torch.device) -> np.ndarray:
    ...
    for y in range(H):
        ...
        prefix_as = [0]
        while x < W:
            label_probs, _ = _next_probs(model, prefix_tokens, prefix_types, prefix_al, prefix_as, use_2d_context, device)
            ...
            _lp, len_probs = _next_probs(model, prefix_tokens, prefix_types, prefix_al, prefix_as, use_2d_context, device)
            allowed = np.zeros((model.wmax + 1,), dtype=bool)
            ...
            if run_len < 1 or run_len > (W - x):
                raise ValueError(f"Decoded invalid run length {run_len} at row={y}, x={x}")
        ...
        out[y] = decode_row_runs(list(zip(labels_row, lens_row)), W, dtype=out.dtype)
        ...
        if online.mode == "online":
            ...
            if online.online_after == "row":
                for _ in range(online.steps_per_row):
                    _online_update_row(model, optimizer, *row_tup, no_above_idx, online.clip, use_2d_context, device)
...
def load_checkpoint_and_model(checkpoint_path: Path, use_2d_context: bool, device: torch.device) -> Tuple[dict, LMEntropyModel]:
    ckpt = torch.load(checkpoint_path, map_location="cpu")
    cfg = ckpt["config"]
    ckpt_use_2d = bool(cfg.get("use_2d_context", False))
    if use_2d_context != ckpt_use_2d:
        raise ValueError(f"use_2d_context mismatch: checkpoint={ckpt_use_2d} cli={use_2d_context}")
    return ckpt, _build_model(ckpt, use_2d_context, device)


def build_header(mask: np.ndarray, ckpt: dict, checkpoint_path: Path, use_2d_context: bool, online: OnlineConfig) -> dict:
    cfg = ckpt["config"]
    return {
        "H": int(mask.shape[0]),
        "W": int(mask.shape[1]),
...
def encode_file(mask_path: Path, checkpoint_path: Path, out_bin: Path, use_2d_context: bool, online: OnlineConfig, device: torch.device, dump_stats: Optional[Path] = None) -> dict:
    set_deterministic(0)
    ...
    payload, ideal_bits, ideal_len_bits = encode_mask(mask, model, labels, label_to_idx, use_2d_context, online, device)
    header = build_header(mask, ckpt, checkpoint_path, use_2d_context, online)
    header_b = json.dumps(header, sort_keys=True).encode("utf-8")
    n = len(header_b)
    uv = bytearray()
    while True:
        b = n & 0x7F
    ...
    stats = {
        ...
        "actual_bits_with_header": 8 * (len(payload) + len(header_b) + len(uv)),
        "ideal_len_bits": ideal_len_bits,
    }
    stats["overhead_bits"] = stats["actual_bits_payload"] - stats["ideal_bits"]
    stats["overhead_pct"] = (stats["overhead_bits"] / stats["ideal_bits"] * 100.0) if stats["ideal_bits"] > 0 else 0.0
...
def read_bin(path: Path) -> Tuple[dict, bytes]:
    ...
    while True:
        ...
        if (x & 0x80) == 0:
            break
        shift += 7
    header = json.loads(b[idx : idx + n].decode("utf-8"))
    payload = b[idx + n :]
